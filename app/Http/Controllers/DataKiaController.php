...
class DataKiaController extends Controller
{
    ...
    {
        abort_unless(
            auth()->check() && auth()->user()->role === "pengguna",
            403,
        );
        ...
        return view(
            "pengguna.bukuKIA",
            compact("dataKia", "bukuKiaCards", "totalBukuKia", "activeKia"),
        );
    }
    ...
    public function saveWizard(Request $request)
    {
        ...
        $dataKia->layanan()->updateOrCreate(
            ...
            [
                ...
                "tanggal_berlaku_asuransi_suami" => $clean(
                    $request->tanggal_berlaku_asuransi_suami,
                ),
                "puskesmas_domisili_suami" => $clean(
                    $request->puskesmas_domisili_suami,
                ),
                "no_catatan_medik_rs_suami" => $clean(
                    $request->no_catatan_medik_rs_suami,
                ),

                // Anak
                "asuransi_anak" => $clean($request->asuransi_anak),
                "no_asuransi_anak" => $clean($request->no_asuransi_anak),
                "tanggal_berlaku_asuransi_anak" => $clean(
                    $request->tanggal_berlaku_asuransi_anak,
                ),
                "puskesmas_domisili_anak" => $clean(
                    $request->puskesmas_domisili_anak,
                ),
                "no_catatan_medik_rs_anak" => $clean(
                    $request->no_catatan_medik_rs_anak,
                ),
            ],
        );
        ...
        for ($bulan = 1; $bulan <= 9; $bulan++) {
            $ttdData = [
                "usia_kehamilan" => $clean(
                    $request->input("ttd_bulan_{$bulan}_usia"),
                ),
                "bulan_tahun" => $clean(
                    $request->input("ttd_bulan_{$bulan}_bulan_tahun"),
                ),
            ];
            for ($hari = 1; $hari <= 31; $hari++) {
                $ttdData["h{$hari}"] = $request->has(
                    "ttd_bulan_{$bulan}_h{$hari}",
                );
            }
            $dataKia
                ->ttdTrackings()
                ->updateOrCreate(["bulan_ke" => $bulan], $ttdData);
        }
        ...
        for ($minggu = 1; $minggu <= 42; $minggu++) {
            ...
            foreach ($pemantauanFields as $field) {
                $data[$field] = $request->has(
                    "pemantauan_minggu_{$minggu}_{$colIndex}",
                );
                $colIndex++;
            }
            $dataKia
                ->pemantauanMingguans()
                ->updateOrCreate(["minggu_ke" => $minggu], $data);
        }
        ...
        for ($sesi = 1; $sesi <= 9; $sesi++) {
            $dataKia->absenKelasIbuHamils()->updateOrCreate(
                ...
                [
                    "tanggal" => $clean(
                        $request->input("kelas_ibu_tanggal_{$sesi}"),
                    ),
                    "kader_info" => $clean(
                        $request->input("kelas_ibu_kader_{$sesi}"),
                    ),
                ],
            );
        }
        ...
        $dataKia
            ->persiapanMelahirkan()
            ->updateOrCreate(["data_kia_id" => $dataKia->id], $persiapanData);
        ...
        for ($hari = 1; $hari <= 42; $hari++) {
            ...
            $data["paraf_kader_nakes"] = $clean(
                $request->input("nifas_paraf_{$hari}"),
            );
            $dataKia
                ->pemantauanIbuNifas()
                ->updateOrCreate(["hari_ke" => $hari], $data);
        }
        ...
        for ($hari = 1; $hari <= 28; $hari++) {
            ...
            $data["paraf_kader_nakes"] = $clean(
                $request->input("bayi_paraf_{$hari}"),
            );
            $dataKia
                ->pemantauanBayis()
                ->updateOrCreate(
        ...
        $dataKia->warnaTinja()->updateOrCreate(
            ...
            [
                ...
                "tanggal_2_4_bulan" => $clean(
                    $request->tinja_2_4_bulan_tanggal,
                ),
        ...
        for ($sesi = 1; $sesi <= 9; $sesi++) {
            $dataKia->absenKelasBalitas()->updateOrCreate(
                ...
                [
                    "tanggal" => $clean(
                        $request->input("kelas_balita_tanggal_{$sesi}"),
                    ),
                    "kader_info" => $clean(
                        $request->input("kelas_balita_kader_{$sesi}"),
                    ),
                ],
            );
        }
        ...
        for ($minggu = 1; $minggu <= 8; $minggu++) {
            ...
            foreach ($mingguanBayiFields as $field) {
                $data[$field] = $request->has(
                    "mingguan_bayi_{$minggu}_{$colIndex}",
                );
                $colIndex++;
            }
            $data["paraf_kader_nakes"] = $clean(
                $request->input("mingguan_bayi_paraf_{$minggu}"),
            );
            $dataKia
                ->pemantauanMingguanBayis()
                ->updateOrCreate(
        ...
        $dataKia->perkembanganBayi()->updateOrCreate(
            ["data_kia_id" => $dataKia->id],
            [
                "angkat_kepala_45" => $request->has("bayi_angkat_kepala_45")
                    ? $request->bayi_angkat_kepala_45 === "1"
                    : null,
                "gerak_kepala" => $request->has("bayi_gerakkan_kepala")
                    ? $request->bayi_gerakkan_kepala === "1"
                    : null,
                "tatap_wajah" => $request->has("bayi_tatap_wajah")
                    ? $request->bayi_tatap_wajah === "1"
                    : null,
                "ngoceh" => $request->has("bayi_ngoceh")
                    ? $request->bayi_ngoceh === "1"
                    : null,
                "tertawa_keras" => $request->has("bayi_tertawa_keras")
                    ? $request->bayi_tertawa_keras === "1"
                    : null,
                "terkejut_suara" => $request->has("bayi_terkejut_suara")
                    ? $request->bayi_terkejut_suara === "1"
                    : null,
                "tersenyum" => $request->has("bayi_tersenyum")
                    ? $request->bayi_tersenyum === "1"
                    : null,
                "mengenal_ibu" => $request->has("bayi_mengenal_ibu")
                    ? $request->bayi_mengenal_ibu === "1"
                    : null,
            ],
        );
        ...
        $dataKia->perkembanganBayi6Bulan()->updateOrCreate(
            ["data_kia_id" => $dataKia->id],
            [
                "berbalik" => $request->has("bayi_berbalik")
                    ? $request->bayi_berbalik === "1"
                    : null,
                "kepala_tegak_90" => $request->has("bayi_kepala_tegak_90")
                    ? $request->bayi_kepala_tegak_90 === "1"
                    : null,
                "kepala_stabil" => $request->has("bayi_kepala_stabil")
                    ? $request->bayi_kepala_stabil === "1"
                    : null,
                "genggam_mainan" => $request->has("bayi_genggam_mainan")
                    ? $request->bayi_genggam_mainan === "1"
                    : null,
                "raih_benda" => $request->has("bayi_raih_benda")
                    ? $request->bayi_raih_benda === "1"
                    : null,
                "amati_tangan" => $request->has("bayi_amati_tangan")
                    ? $request->bayi_amati_tangan === "1"
                    : null,
                "luas_pandang" => $request->has("bayi_luas_pandang")
                    ? $request->bayi_luas_pandang === "1"
                    : null,
                "arah_mata" => $request->has("bayi_arah_mata")
                    ? $request->bayi_arah_mata === "1"
                    : null,
                "suara_gembira" => $request->has("bayi_suara_gembira")
                    ? $request->bayi_suara_gembira === "1"
                    : null,
                "senyum_mainan" => $request->has("bayi_senyum_mainan")
                    ? $request->bayi_senyum_mainan === "1"
                    : null,
            ],
        );
        ...
        for ($bulan = 1; $bulan <= 6; $bulan++) {
            ...
            foreach ($bulananBayiFields as $field) {
                $data[$field] = $request->has(
                    "bulanan_bayi_{$bulan}_{$colIndex}",
                );
                $colIndex++;
            }
            $data["paraf_kader_nakes"] = $clean(
                $request->input("bulanan_bayi_paraf_{$bulan}"),
            );
            $dataKia
                ->pemantauanBulananBayis()
                ->updateOrCreate(
        ...
        $kesehatanLingkunganFields = [
            ...
            "ctps_sabun",
            "ctps_sebelum_makan",
            "ctps_sebelum_mengolah",
            "ctps_sebelum_menyusui",
            "ctps_setelah_bab",
            "sumber_air_pipa",
            "sumber_air_kran",
            "sumber_air_sumur",
        ...
        $dataKia
            ->kesehatanLingkungan()
            ->updateOrCreate(["data_kia_id" => $dataKia->id], $lingkunganData);
    ...
    public function exportPdf($id)
    {
        ...
        if ($user->role === "pengguna") {
            ...
            abort_unless(
                in_array($user->role, ["admin", "bidan", "dokter"]),
                403,
            );
        }
        ...
        return response($pdfContent, 200, [
            ...
            "Content-Disposition" =>
                'inline; filename="Buku_KIA_' .
                ($dataKia->ibu->nama ?? "Identitas") .
                '.pdf"',
        ]);
    }
    ...
    public function saveRiwayat(Request $request, $id)
    {
        ...
        $dataKia->riwayat()->updateOrCreate(
            ...
            [
                ...
                "jumlah_anak_hidup" => $clean(
                    $request->jumlah_anak_lahir_hidup,
                ),
                "riwayat_keguguran" => $clean($request->riwayat_keguguran),
                "riwayat_penyakit_ibu" => $clean(
                    $request->riwayat_penyakit_ibu,
                ),
            ],
        );
    ...
    public function editPelayanan($id)
    {
        ...
        return view(
            "nakes.kia-edit-pelayanan",
            compact("dataKia", "role", "pelayanan"),
        );
    }
    ...
    public function savePelayanan(Request $request, $id)
    {
        ...
        $tripelCombined = null;
        if (
            $request->has("tripel_eliminasi_h") ||
            $request->has("tripel_eliminasi_s") ||
            $request->has("tripel_eliminasi_hep_b")
        ) {
            $tripelCombined =
                ($request->tripel_eliminasi_h ?? "") .
                "," .
                ($request->tripel_eliminasi_s ?? "") .
                "," .
                ($request->tripel_eliminasi_hep_b ?? "");
            if ($tripelCombined === ",,") {
                $tripelCombined = null;
            }
        }
        ...
        return back()
            ->with(
                "success",
                "Pelayanan Kesehatan Ibu kunjungan ke-" .
                    $kunjunganKe .
                    " berhasil disimpan.",
            )
            ->with("active_tab", "kunjungan" . $kunjunganKe);
    }
    ...
    public function editEvaluasi($id)
    {
        ...
        $pemeriksaan =
            $dataKia->pemeriksaanTrimester1 ??
            new \App\Models\KiaPemeriksaanTrimester1();
    ...
    public function saveEvaluasi(Request $request, $id)
    {
        ...
        $dataKia->evaluasiKesehatanIbu()->updateOrCreate(
            ...
            [
                ...
                "riwayat_kesehatan_ibu" =>
                    $request->riwayat_kesehatan_ibu ?? [],
                "riwayat_kesehatan_ibu_lainnya" =>
                    $request->riwayat_kesehatan_ibu_lainnya,
                "riwayat_perilaku" => $request->riwayat_perilaku ?? [],
                "riwayat_perilaku_lainnya" =>
                    $request->riwayat_perilaku_lainnya,
                "riwayat_penyakit_keluarga" =>
                    $request->riwayat_penyakit_keluarga ?? [],
                "riwayat_penyakit_keluarga_lainnya" =>
                    $request->riwayat_penyakit_keluarga_lainnya,
        ...
        return back()->with(
            "success",
            "Data Evaluasi Halaman 51 berhasil disimpan!",
        );
    }
    ...
    public function saveTrimester1(Request $request, $id)
    {
        ...
        if ($request->filled("deleted_catatan")) {
            ...
            \App\Models\KiaCatatanPelayananTrimester1::whereIn(
                "id",
                $deletedIds,
            )
                ->where("data_kia_id", $dataKia->id)
                ->delete();
        }
        ...
        if ($request->has("catatan")) {
            foreach ($request->catatan as $cat) {
                $catatanModel = isset($cat["id"])
                    ? \App\Models\KiaCatatanPelayananTrimester1::find(
                        $cat["id"],
                    )
                    : new \App\Models\KiaCatatanPelayananTrimester1();
                $catatanModel->data_kia_id = $dataKia->id;
                $catatanModel->tanggal_periksa =
                    $cat["tanggal_periksa"] ?? null;
                $catatanModel->catatan = $cat["catatan"] ?? null;
                $catatanModel->tanggal_kembali =
                    $cat["tanggal_kembali"] ?? null;
                $catatanModel->save();
            }
        }
        ...
        return back()->with(
            "success",
            "Data Trimester 1 (Halaman 52-53) berhasil disimpan!",
        );
    }
    public function editTrimester2($id)
    {
        ...
        $pemeriksaan =
            $dataKia->pemeriksaanTrimester2 ??
            new \App\Models\KiaPemeriksaanTrimester2();
    ...
    public function saveTrimester2(Request $request, $id)
    {
        ...
        $pemeriksaanModel =
            ...
            new \App\Models\KiaPemeriksaanTrimester2();
        $pemeriksaanModel->data_kia_id = $dataKia->id;
        $pemeriksaanModel->skrining_preeklampsia =
            $request->skrining_preeklampsia;
        $pemeriksaanModel->kesimpulan_preeklampsia =
            $request->kesimpulan_preeklampsia;
        $pemeriksaanModel->lab_gula_darah_puasa =
            $request->lab_gula_darah_puasa;
        ...
        $pemeriksaanModel->tgl_periksa_diabetes =
            $request->tgl_periksa_diabetes;
        $pemeriksaanModel->nama_dokter_diabetes =
            $request->nama_dokter_diabetes;
        $pemeriksaanModel->save();

        if ($request->filled("deleted_catatan")) {
            $deletedIds = explode(",", $request->deleted_catatan);
            \App\Models\KiaCatatanPelayananTrimester2::whereIn(
                "id",
                $deletedIds,
            )
                ->where("data_kia_id", $dataKia->id)
        ...
        if ($request->has("catatan")) {
            foreach ($request->catatan as $cat) {
                $catatanModel = isset($cat["id"])
                    ? \App\Models\KiaCatatanPelayananTrimester2::find(
                        $cat["id"],
                    )
                    : new \App\Models\KiaCatatanPelayananTrimester2();
                $catatanModel->data_kia_id = $dataKia->id;
                $catatanModel->tanggal_periksa =
                    $cat["tanggal_periksa"] ?? null;
                $catatanModel->catatan = $cat["catatan"] ?? null;
        ...
        return back()->with(
            "success",
            "Data Trimester 2 (Halaman 53) berhasil disimpan!",
        );
    }
    ...
    public function ttdIndex()
    {
        ...
        $dataKia = DataKia::with("ttdTrackings")->firstOrCreate([
            "user_id" => auth()->id(),
        ]);
    ...
    public function ttdStore(Request $request)
    {
        ...
        $dataKia
            ->ttdTrackings()
            ->updateOrCreate(["bulan_ke" => $bulanKe], $data);

        return back()->with(
            "success",
            "Catatan minum TTD bulan ke-" . $bulanKe . " berhasil disimpan.",
        );
    }
    ...
    public function pemantauanIndex()
    {
        ...
        $dataKia = DataKia::with("pemantauanMingguans")->firstOrCreate([
            "user_id" => auth()->id(),
        ]);
        $pemantauans = $dataKia->pemantauanMingguans->keyBy("minggu_ke");

        return view(
            "pengguna.kia-pemantauan",
            compact("dataKia", "pemantauans"),
        );
    }
    ...
    public function pemantauanStore(Request $request)
    {
        ...
        $dataKia
            ->pemantauanMingguans()
            ->updateOrCreate(["minggu_ke" => $mingguKe], $data);

        return back()->with(
            "success",
            "Catatan pemantauan minggu ke-" . $mingguKe . " berhasil disimpan.",
        );
    }
    ...
    {
        $userId = auth()->id();
        $dataKia = DataKia::with("absenKelasIbuHamils")
        ...
        if (!$dataKia) {
            return redirect()
                ...
                ->with(
                    "info",
                    "Silakan lengkapi screening Buku KIA terlebih dahulu.",
                );
        }
    ...
    public function kelasIbuStore(Request $request)
    {
        ...
        return back()->with(
            "success",
            "Data absensi kelas ibu hamil ke-" .
                $request->kehadiran_ke .
                " berhasil disimpan.",
        );
    }
    ...
    {
        $userId = auth()->id();
        $dataKia = DataKia::with("persiapanMelahirkan")
            ->where("user_id", $userId)
            ->first();
    ...
    public function persiapanStore(Request $request)
    {
        ...
        $dataKia
            ->persiapanMelahirkan()
            ->updateOrCreate(["data_kia_id" => $dataKia->id], $data);

        return back()->with(
            "success",
            "Persiapan melahirkan berhasil disimpan.",
        );
    }
    ...
    {
        $userId = auth()->id();
        $dataKia = DataKia::with("pemantauanIbuNifas")
            ->where("user_id", $userId)
            ->first();
        ...
        return view(
            "pengguna.kia-pemantauan-nifas",
            compact("dataKia", "records"),
        );
    }
    ...
    public function nifasStore(Request $request)
    {
        ...
        $dataKia
            ->pemantauanIbuNifas()
            ->updateOrCreate(["hari_ke" => $request->hari_ke], $data);

        return back()->with(
            "success",
            "Catatan pemantauan ibu nifas hari ke-" .
                $request->hari_ke .
                " berhasil disimpan.",
        );
    }
    ...
    {
        $userId = auth()->id();
        $dataKia = DataKia::with("keluargaBerencana")
            ->where("user_id", $userId)
            ->first();
    ...
    public function kbStore(Request $request)
    {
        ...
        return back()->with(
            "success",
            "Catatan rencana Keluarga Berencana (KB) berhasil disimpan.",
        );
    }
    ...
    {
        $userId = auth()->id();
        $dataKia = DataKia::with("bayiBaruLahir")
            ->where("user_id", $userId)
            ->first();
    ...
    public function bayiStore(Request $request)
    {
        ...
        return back()->with(
            "success",
            "Ceklist pemeriksaan bayi baru lahir berhasil disimpan.",
        );
    }
    ...
    {
        $userId = auth()->id();
        $dataKia = DataKia::with("pemantauanBayis")
            ->where("user_id", $userId)
            ->first();
    ...
    public function pemantauanBayiStore(Request $request)
    {
        ...
        return back()->with(
            "success",
            "Catatan pemantauan harian bayi hari ke-" .
                $request->hari_ke .
                " berhasil disimpan.",
        );
    }
    ...
    {
        $userId = auth()->id();
        $dataKia = DataKia::with("warnaTinja")
            ->where("user_id", $userId)
            ->first();
    ...
    public function warnaTinjaStore(Request $request)
    {
        ...
        return back()->with(
            "success",
            "Pemantauan warna tinja bayi berhasil disimpan.",
        );
    }
    ...
    {
        $userId = auth()->id();
        $dataKia = DataKia::with("absenKelasBalitas")
            ->where("user_id", $userId)
            ->first();
    ...
    public function kelasBalitaStore(Request $request)
    {
        ...
        return back()->with(
            "success",
            "Absensi kehadiran kelas ibu balita sesi ke-" .
                $request->kehadiran_ke .
                " berhasil disimpan.",
        );
    }
    ...
    public function pemantauanMingguanBayiIndex()
    {
        ...
        return view(
            "pengguna.kia-mingguan-bayi",
            compact("dataKia", "mingguan", "perkembangan"),
        );
    }
    ...
    public function pemantauanMingguanBayiStore(Request $request)
    {
        ...
        return back()
            ->with(
                "success",
                "Catatan pemantauan mingguan bayi minggu ke-" .
                    $request->minggu_ke .
                    " berhasil disimpan.",
            )
    ...
    public function perkembanganBayiStore(Request $request)
    {
        ...
        $dataKia->perkembanganBayi()->updateOrCreate(
            ["data_kia_id" => $dataKia->id],
            [
                "angkat_kepala_45" => $request->has("angkat_kepala_45")
                    ? $request->angkat_kepala_45 === "1"
                    : null,
                "gerak_kepala" => $request->has("gerak_kepala")
                    ? $request->gerak_kepala === "1"
                    : null,
                "tatap_wajah" => $request->has("tatap_wajah")
                    ? $request->tatap_wajah === "1"
                    : null,
                "ngoceh" => $request->has("ngoceh")
                    ? $request->ngoceh === "1"
                    : null,
                "tertawa_keras" => $request->has("tertawa_keras")
                    ? $request->tertawa_keras === "1"
                    : null,
                "terkejut_suara" => $request->has("terkejut_suara")
                    ? $request->terkejut_suara === "1"
                    : null,
                "tersenyum" => $request->has("tersenyum")
                    ? $request->tersenyum === "1"
                    : null,
                "mengenal_ibu" => $request->has("mengenal_ibu")
                    ? $request->mengenal_ibu === "1"
                    : null,
            ],
        );

        return back()
            ->with("success", "Checklist perkembangan bayi berhasil disimpan.")
    ...
    public function pemantauanBulananBayiIndex()
    {
        ...
        return view(
            "pengguna.kia-bulanan-bayi",
            compact("dataKia", "bulanan", "perkembangan"),
        );
    }
    ...
    public function pemantauanBulananBayiStore(Request $request)
    {
        ...
        return back()
            ->with(
                "success",
                "Catatan pemantauan bulanan bayi bulan ke-" .
                    $request->bulan_ke .
                    " berhasil disimpan.",
            )
    ...
    public function perkembanganBayi6BulanStore(Request $request)
    {
        ...
        $dataKia->perkembanganBayi6Bulan()->updateOrCreate(
            ["data_kia_id" => $dataKia->id],
            [
                "berbalik" => $request->has("berbalik")
                    ? $request->berbalik === "1"
                    : null,
                "kepala_tegak_90" => $request->has("kepala_tegak_90")
                    ? $request->kepala_tegak_90 === "1"
                    : null,
                "kepala_stabil" => $request->has("kepala_stabil")
                    ? $request->kepala_stabil === "1"
                    : null,
                "genggam_mainan" => $request->has("genggam_mainan")
                    ? $request->genggam_mainan === "1"
                    : null,
                "raih_benda" => $request->has("raih_benda")
                    ? $request->raih_benda === "1"
                    : null,
                "amati_tangan" => $request->has("amati_tangan")
                    ? $request->amati_tangan === "1"
                    : null,
                "luas_pandang" => $request->has("luas_pandang")
                    ? $request->luas_pandang === "1"
                    : null,
                "arah_mata" => $request->has("arah_mata")
                    ? $request->arah_mata === "1"
                    : null,
                "suara_gembira" => $request->has("suara_gembira")
                    ? $request->suara_gembira === "1"
                    : null,
                "senyum_mainan" => $request->has("senyum_mainan")
                    ? $request->senyum_mainan === "1"
                    : null,
            ],
        );
        ...
        return back()
            ->with(
                "success",
                "Checklist perkembangan bayi umur 3-6 bulan berhasil disimpan.",
            )
            ->with("active_tab", "perkembangan");
    }
    ...
    public function pemantauanBulananBayi12Index()
    {
        ...
        return view(
            "pengguna.kia-bulanan-bayi-12",
            compact("dataKia", "bulanan", "perkembangan9", "perkembangan12"),
        );
    }
    ...
    public function perkembanganBayi9BulanStore(Request $request)
    {
        ...
        $dataKia->perkembanganBayi9Bulan()->updateOrCreate(
            ["data_kia_id" => $dataKia->id],
            [
                "duduk_mandiri" => $request->has("duduk_mandiri")
                    ? $request->duduk_mandiri === "1"
                    : null,
                "tengkurap_dada" => $request->has("tengkurap_dada")
                    ? $request->tengkurap_dada === "1"
                    : null,
                "merangkak" => $request->has("merangkak")
                    ? $request->merangkak === "1"
                    : null,
                "pindah_benda" => $request->has("pindah_benda")
                    ? $request->pindah_benda === "1"
                    : null,
                "pungut_2_benda" => $request->has("pungut_2_benda")
                    ? $request->pungut_2_benda === "1"
                    : null,
                "pungut_kacang" => $request->has("pungut_kacang")
                    ? $request->pungut_kacang === "1"
                    : null,
                "bersuara_tanpa_arti" => $request->has("bersuara_tanpa_arti")
                    ? $request->bersuara_tanpa_arti === "1"
                    : null,
                "cari_mainan" => $request->has("cari_mainan")
                    ? $request->cari_mainan === "1"
                    : null,
                "tepuk_tangan" => $request->has("tepuk_tangan")
                    ? $request->tepuk_tangan === "1"
                    : null,
                "lempar_benda" => $request->has("lempar_benda")
                    ? $request->lempar_benda === "1"
                    : null,
                "makan_kue" => $request->has("makan_kue")
                    ? $request->makan_kue === "1"
                    : null,
            ],
        );
        ...
        return back()
            ->with(
                "success",
                "Checklist perkembangan bayi umur 6-9 bulan berhasil disimpan.",
            )
    ...
    public function perkembanganBayi12BulanStore(Request $request)
    {
        ...
        $dataKia->perkembanganBayi12Bulan()->updateOrCreate(
            ["data_kia_id" => $dataKia->id],
            [
                "angkat_badan_berdiri" => $request->has("angkat_badan_berdiri")
                    ? $request->angkat_badan_berdiri === "1"
                    : null,
                "belajar_berdiri" => $request->has("belajar_berdiri")
                    ? $request->belajar_berdiri === "1"
                    : null,
                "jalan_dituntun" => $request->has("jalan_dituntun")
                    ? $request->jalan_dituntun === "1"
                    : null,
                "ulur_tangan_raih" => $request->has("ulur_tangan_raih")
                    ? $request->ulur_tangan_raih === "1"
                    : null,
                "genggam_pensil" => $request->has("genggam_pensil")
                    ? $request->genggam_pensil === "1"
                    : null,
                "masuk_benda_mulut" => $request->has("masuk_benda_mulut")
                    ? $request->masuk_benda_mulut === "1"
                    : null,
                "tiru_bunyi" => $request->has("tiru_bunyi")
                    ? $request->tiru_bunyi === "1"
                    : null,
                "sebut_2_suku_kata" => $request->has("sebut_2_suku_kata")
                    ? $request->sebut_2_suku_kata === "1"
                    : null,
                "eksplorasi_sekitar" => $request->has("eksplorasi_sekitar")
                    ? $request->eksplorasi_sekitar === "1"
                    : null,
                "reaksi_panggilan" => $request->has("reaksi_panggilan")
                    ? $request->reaksi_panggilan === "1"
                    : null,
                "bermain_cilukba" => $request->has("bermain_cilukba")
                    ? $request->bermain_cilukba === "1"
                    : null,
                "kenal_keluarga" => $request->has("kenal_keluarga")
                    ? $request->kenal_keluarga === "1"
                    : null,
            ],
        );
        ...
        return back()
            ->with(
                "success",
                "Checklist perkembangan bayi umur 9-12 bulan berhasil disimpan.",
            )
    ...
    public function pemantauanBulananAnak24Index()
    {
        ...
        return view(
            "pengguna.kia-bulanan-anak-24",
            compact("dataKia", "bulanan", "perkembangan18", "perkembangan24"),
        );
    }
    ...
    public function pemantauanBulananAnak24Store(Request $request)
    {
        ...
        return back()
            ->with(
                "success",
                "Catatan pemantauan bulanan anak bulan ke-" .
                    $request->bulan_ke .
                    " berhasil disimpan.",
            )
            ->with("active_tab", "bulanan")
    ...
    public function perkembanganBayi18BulanStore(Request $request)
    {
        ...
        $dataKia->perkembanganBayi18Bulan()->updateOrCreate(
            ["data_kia_id" => $dataKia->id],
            [
                "berdiri_tanpa_pegangan" => $request->has(
                    "berdiri_tanpa_pegangan",
                )
                    ? $request->berdiri_tanpa_pegangan === "1"
                    : null,
                "bungkuk_pungut_mainan" => $request->has(
                    "bungkuk_pungut_mainan",
                )
                    ? $request->bungkuk_pungut_mainan === "1"
                    : null,
                "jalan_mundur_5_langkah" => $request->has(
                    "jalan_mundur_5_langkah",
                )
                    ? $request->jalan_mundur_5_langkah === "1"
                    : null,
                "panggil_papa_mama" => $request->has("panggil_papa_mama")
                    ? $request->panggil_papa_mama === "1"
                    : null,
                "tumpuk_2_kubus" => $request->has("tumpuk_2_kubus")
                    ? $request->tumpuk_2_kubus === "1"
                    : null,
                "masuk_kubus_kotak" => $request->has("masuk_kubus_kotak")
                    ? $request->masuk_kubus_kotak === "1"
                    : null,
                "tunjuk_tanpa_nangis" => $request->has("tunjuk_tanpa_nangis")
                    ? $request->tunjuk_tanpa_nangis === "1"
                    : null,
                "rasa_cemburu" => $request->has("rasa_cemburu")
                    ? $request->rasa_cemburu === "1"
                    : null,
            ],
        );
        ...
        return back()
            ->with(
                "success",
                "Checklist perkembangan bayi umur 12 - 18 bulan berhasil disimpan.",
            )
    ...
    public function perkembanganBayi24BulanStore(Request $request)
    {
        ...
        $dataKia->perkembanganBayi24Bulan()->updateOrCreate(
            ["data_kia_id" => $dataKia->id],
            [
                "berdiri_30_detik" => $request->has("berdiri_30_detik")
                    ? $request->berdiri_30_detik === "1"
                    : null,
                "jalan_tanpa_huyung" => $request->has("jalan_tanpa_huyung")
                    ? $request->jalan_tanpa_huyung === "1"
                    : null,
                "tumpuk_4_kubus" => $request->has("tumpuk_4_kubus")
                    ? $request->tumpuk_4_kubus === "1"
                    : null,
                "pungut_benda_kecil" => $request->has("pungut_benda_kecil")
                    ? $request->pungut_benda_kecil === "1"
                    : null,
                "gelinding_bola" => $request->has("gelinding_bola")
                    ? $request->gelinding_bola === "1"
                    : null,
                "sebut_3_6_kata" => $request->has("sebut_3_6_kata")
                    ? $request->sebut_3_6_kata === "1"
                    : null,
                "bantu_pekerjaan_rumah" => $request->has(
                    "bantu_pekerjaan_rumah",
                )
                    ? $request->bantu_pekerjaan_rumah === "1"
                    : null,
                "pegang_cangkir_sendiri" => $request->has(
                    "pegang_cangkir_sendiri",
                )
                    ? $request->pegang_cangkir_sendiri === "1"
                    : null,
            ],
        );
        ...
        return back()
            ->with(
                "success",
                "Checklist perkembangan bayi umur 18 - 24 bulan berhasil disimpan.",
            )
    ...
    public function perkembanganAnak36BulanStore(Request $request)
    {
        ...
        $dataKia->perkembanganAnak36Bulan()->updateOrCreate(
            ["data_kia_id" => $dataKia->id],
            [
                "naik_tangga" => $request->has("naik_tangga")
                    ? $request->naik_tangga === "1"
                    : null,
                "tendang_bola" => $request->has("tendang_bola")
                    ? $request->tendang_bola === "1"
                    : null,
                "coret_kertas" => $request->has("coret_kertas")
                    ? $request->coret_kertas === "1"
                    : null,
                "bicara_2_kata" => $request->has("bicara_2_kata")
                    ? $request->bicara_2_kata === "1"
                    : null,
                "tunjuk_bagian_tubuh" => $request->has("tunjuk_bagian_tubuh")
                    ? $request->tunjuk_bagian_tubuh === "1"
                    : null,
                "sebut_nama_benda" => $request->has("sebut_nama_benda")
                    ? $request->sebut_nama_benda === "1"
                    : null,
                "pungut_mainan" => $request->has("pungut_mainan")
                    ? $request->pungut_mainan === "1"
                    : null,
                "makan_nasi_sendiri" => $request->has("makan_nasi_sendiri")
                    ? $request->makan_nasi_sendiri === "1"
                    : null,
                "lepas_pakaian" => $request->has("lepas_pakaian")
                    ? $request->lepas_pakaian === "1"
                    : null,
            ],
        );
        ...
        return back()
            ->with(
                "success",
                "Checklist perkembangan anak umur 2 - 3 tahun berhasil disimpan.",
            )
    ...
    public function perkembanganAnak48BulanStore(Request $request)
    {
        ...
        $dataKia->perkembanganAnak48Bulan()->updateOrCreate(
            ["data_kia_id" => $dataKia->id],
            [
                "berdiri_1_kaki_2_detik" => $request->has(
                    "berdiri_1_kaki_2_detik",
                )
                    ? $request->berdiri_1_kaki_2_detik === "1"
                    : null,
                "lompat_kedua_kaki" => $request->has("lompat_kedua_kaki")
                    ? $request->lompat_kedua_kaki === "1"
                    : null,
                "kayuh_sepeda_roda_3" => $request->has("kayuh_sepeda_roda_3")
                    ? $request->kayuh_sepeda_roda_3 === "1"
                    : null,
                "gambar_garis_lurus" => $request->has("gambar_garis_lurus")
                    ? $request->gambar_garis_lurus === "1"
                    : null,
                "tumpuk_8_kubus" => $request->has("tumpuk_8_kubus")
                    ? $request->tumpuk_8_kubus === "1"
                    : null,
                "kenal_2_4_warna" => $request->has("kenal_2_4_warna")
                    ? $request->kenal_2_4_warna === "1"
                    : null,
                "sebut_nama_umur_tempat" => $request->has(
                    "sebut_nama_umur_tempat",
                )
                    ? $request->sebut_nama_umur_tempat === "1"
                    : null,
                "mengerti_arti_kata_posisi" => $request->has(
                    "mengerti_arti_kata_posisi",
                )
                    ? $request->mengerti_arti_kata_posisi === "1"
                    : null,
                "dengar_cerita" => $request->has("dengar_cerita")
                    ? $request->dengar_cerita === "1"
                    : null,
                "cuci_tangan_sendiri" => $request->has("cuci_tangan_sendiri")
                    ? $request->cuci_tangan_sendiri === "1"
                    : null,
                "bermain_dengan_teman" => $request->has("bermain_dengan_teman")
                    ? $request->bermain_dengan_teman === "1"
                    : null,
                "pakai_sepatu_sendiri" => $request->has("pakai_sepatu_sendiri")
                    ? $request->pakai_sepatu_sendiri === "1"
                    : null,
                "pakai_celana_baju_sendiri" => $request->has(
                    "pakai_celana_baju_sendiri",
                )
                    ? $request->pakai_celana_baju_sendiri === "1"
                    : null,
            ],
        );
        ...
        return back()
            ->with(
                "success",
                "Checklist perkembangan anak umur 3 - 4 tahun berhasil disimpan.",
            )
    ...
    public function perkembanganAnak60BulanStore(Request $request)
    {
        ...
        $dataKia->perkembanganAnak60Bulan()->updateOrCreate(
            ["data_kia_id" => $dataKia->id],
            [
                "berdiri_1_kaki_6_detik" => $request->has(
                    "berdiri_1_kaki_6_detik",
                )
                    ? $request->berdiri_1_kaki_6_detik === "1"
                    : null,
                "lompat_1_kaki" => $request->has("lompat_1_kaki")
                    ? $request->lompat_1_kaki === "1"
                    : null,
                "menari" => $request->has("menari")
                    ? $request->menari === "1"
                    : null,
                "gambar_tanda_silang" => $request->has("gambar_tanda_silang")
                    ? $request->gambar_tanda_silang === "1"
                    : null,
                "gambar_lingkaran" => $request->has("gambar_lingkaran")
                    ? $request->gambar_lingkaran === "1"
                    : null,
                "gambar_orang_3_bagian" => $request->has(
                    "gambar_orang_3_bagian",
                )
                    ? $request->gambar_orang_3_bagian === "1"
                    : null,
                "kancing_baju_boneka" => $request->has("kancing_baju_boneka")
                    ? $request->kancing_baju_boneka === "1"
                    : null,
                "sebut_nama_lengkap" => $request->has("sebut_nama_lengkap")
                    ? $request->sebut_nama_lengkap === "1"
                    : null,
                "senang_sebut_kata_baru" => $request->has(
                    "senang_sebut_kata_baru",
                )
                    ? $request->senang_sebut_kata_baru === "1"
                    : null,
                "senang_bertanya" => $request->has("senang_bertanya")
                    ? $request->senang_bertanya === "1"
                    : null,
                "jawab_pertanyaan_kata_benar" => $request->has(
                    "jawab_pertanyaan_kata_benar",
                )
                    ? $request->jawab_pertanyaan_kata_benar === "1"
                    : null,
                "bicara_mudah_dimengerti" => $request->has(
                    "bicara_mudah_dimengerti",
                )
                    ? $request->bicara_mudah_dimengerti === "1"
                    : null,
                "banding_ukuran_bentuk" => $request->has(
                    "banding_ukuran_bentuk",
                )
                    ? $request->banding_ukuran_bentuk === "1"
                    : null,
                "sebut_angka_hitung_jari" => $request->has(
                    "sebut_angka_hitung_jari",
                )
                    ? $request->sebut_angka_hitung_jari === "1"
                    : null,
            ],
        );
        ...
        return back()
            ->with(
                "success",
                "Checklist perkembangan anak umur 4 - 5 tahun berhasil disimpan.",
            )
    ...
    public function perkembanganAnak72BulanStore(Request $request)
    {
        ...
        $dataKia->perkembanganAnak72Bulan()->updateOrCreate(
            ["data_kia_id" => $dataKia->id],
            [
                "berjalan_lurus" => $request->has("berjalan_lurus")
                    ? $request->berjalan_lurus === "1"
                    : null,
                "berdiri_1_kaki_11_detik" => $request->has(
                    "berdiri_1_kaki_11_detik",
                )
                    ? $request->berdiri_1_kaki_11_detik === "1"
                    : null,
                "gambar_6_bagian_orang_lengkap" => $request->has(
                    "gambar_6_bagian_orang_lengkap",
                )
                    ? $request->gambar_6_bagian_orang_lengkap === "1"
                    : null,
                "tangkap_bola_kecil" => $request->has("tangkap_bola_kecil")
                    ? $request->tangkap_bola_kecil === "1"
                    : null,
                "gambar_segi_empat" => $request->has("gambar_segi_empat")
                    ? $request->gambar_segi_empat === "1"
                    : null,
                "mengerti_lawan_kata" => $request->has("mengerti_lawan_kata")
                    ? $request->mengerti_lawan_kata === "1"
                    : null,
                "mengerti_pembicaraan_7_kata" => $request->has(
                    "mengerti_pembicaraan_7_kata",
                )
                    ? $request->mengerti_pembicaraan_7_kata === "1"
                    : null,
                "jawab_bahan_guna_benda" => $request->has(
                    "jawab_bahan_guna_benda",
                )
                    ? $request->jawab_bahan_guna_benda === "1"
                    : null,
                "kenal_angka_hitung_5_10" => $request->has(
                    "kenal_angka_hitung_5_10",
                )
                    ? $request->kenal_angka_hitung_5_10 === "1"
                    : null,
                "kenal_warna_warni" => $request->has("kenal_warna_warni")
                    ? $request->kenal_warna_warni === "1"
                    : null,
                "ungkapkan_simpati" => $request->has("ungkapkan_simpati")
                    ? $request->ungkapkan_simpati === "1"
                    : null,
                "ikut_aturan_permainan" => $request->has(
                    "ikut_aturan_permainan",
                )
                    ? $request->ikut_aturan_permainan === "1"
                    : null,
                "pakaian_sendiri_tanpa_bantu" => $request->has(
                    "pakaian_sendiri_tanpa_bantu",
                )
                    ? $request->pakaian_sendiri_tanpa_bantu === "1"
                    : null,
            ],
        );
        ...
        return back()
            ->with(
                "success",
                "Checklist perkembangan anak umur 5 - 6 tahun berhasil disimpan.",
            )
    ...
    {
        $userId = auth()->id();
        $dataKia = DataKia::with("kesehatanLingkungan")
            ->where("user_id", $userId)
            ->first();
        ...
        return view(
            "pengguna.kia-kesehatan-lingkungan",
            compact("dataKia", "lingkungan"),
        );
    }
    ...
    public function kesehatanLingkunganStore(Request $request)
    {
        ...
        $data = [
            "bab_sembarangan" => $request->has("bab_sembarangan")
                ? $request->bab_sembarangan === "1"
                : null,
            "bab_jamban_sendiri" => $request->has("bab_jamban_sendiri")
                ? $request->bab_jamban_sendiri === "1"
                : null,
            "penampung_tangki_septik" => $request->has(
                "penampung_tangki_septik",
            )
                ? $request->penampung_tangki_septik === "1"
                : null,
            "penampung_cubluk" => $request->has("penampung_cubluk")
                ? $request->penampung_cubluk === "1"
                : null,
            "penampung_drainase" => $request->has("penampung_drainase")
                ? $request->penampung_drainase === "1"
                : null,
            "kloset_leher_angsa" => $request->has("kloset_leher_angsa")
                ? $request->kloset_leher_angsa === "1"
                : null,
            "ctps_sarana" => $request->has("ctps_sarana")
                ? $request->ctps_sarana === "1"
                : null,
            "ctps_air_mengalir" => $request->has("ctps_air_mengalir")
                ? $request->ctps_air_mengalir === "1"
                : null,
            "ctps_sabun" => $request->has("ctps_sabun")
                ? $request->ctps_sabun === "1"
                : null,
            "ctps_waktu_sebelum_makan" => $request->has(
                "ctps_waktu_sebelum_makan",
            )
                ? $request->ctps_waktu_sebelum_makan === "1"
                : null,
            "ctps_waktu_sebelum_mengolah" => $request->has(
                "ctps_waktu_sebelum_mengolah",
            )
                ? $request->ctps_waktu_sebelum_mengolah === "1"
                : null,
            "ctps_waktu_sebelum_menyusui" => $request->has(
                "ctps_waktu_sebelum_menyusui",
            )
                ? $request->ctps_waktu_sebelum_menyusui === "1"
                : null,
            "ctps_waktu_setelah_bab" => $request->has("ctps_waktu_setelah_bab")
                ? $request->ctps_waktu_setelah_bab === "1"
                : null,
            "sumber_air_pipa" => $request->has("sumber_air_pipa")
                ? $request->sumber_air_pipa === "1"
                : null,
            "sumber_air_kran" => $request->has("sumber_air_kran")
                ? $request->sumber_air_kran === "1"
                : null,
            "sumber_air_sumur_terlindungi" => $request->has(
                "sumber_air_sumur_terlindungi",
            )
                ? $request->sumber_air_sumur_terlindungi === "1"
                : null,
            "sumber_air_mata_air_terlindungi" => $request->has(
                "sumber_air_mata_air_terlindungi",
            )
                ? $request->sumber_air_mata_air_terlindungi === "1"
                : null,
            "sumber_air_sungai" => $request->has("sumber_air_sungai")
                ? $request->sumber_air_sungai === "1"
                : null,
            "sumber_air_danau" => $request->has("sumber_air_danau")
                ? $request->sumber_air_danau === "1"
                : null,
            "sumber_air_hujan" => $request->has("sumber_air_hujan")
                ? $request->sumber_air_hujan === "1"
                : null,
            "sumber_air_waduk" => $request->has("sumber_air_waduk")
                ? $request->sumber_air_waduk === "1"
                : null,
            "sumber_air_kolam" => $request->has("sumber_air_kolam")
                ? $request->sumber_air_kolam === "1"
                : null,
            "sumber_air_irigasi" => $request->has("sumber_air_irigasi")
                ? $request->sumber_air_irigasi === "1"
                : null,
            "kelola_air_rebus" => $request->has("kelola_air_rebus")
                ? $request->kelola_air_rebus === "1"
                : null,
            "kelola_air_endap_saring" => $request->has(
                "kelola_air_endap_saring",
            )
                ? $request->kelola_air_endap_saring === "1"
                : null,
            "kelola_air_wadah_tertutup" => $request->has(
                "kelola_air_wadah_tertutup",
            )
                ? $request->kelola_air_wadah_tertutup === "1"
                : null,
            "kelola_makanan_tertutup" => $request->has(
                "kelola_makanan_tertutup",
            )
                ? $request->kelola_makanan_tertutup === "1"
                : null,
            "kelola_makanan_jauh_bahan_berbahaya" => $request->has(
                "kelola_makanan_jauh_bahan_berbahaya",
            )
                ? $request->kelola_makanan_jauh_bahan_berbahaya === "1"
                : null,
            "kelola_makanan_baik_benar" => $request->has(
                "kelola_makanan_baik_benar",
            )
                ? $request->kelola_makanan_baik_benar === "1"
                : null,
            "sampah_tidak_berserakan" => $request->has(
                "sampah_tidak_berserakan",
            )
                ? $request->sampah_tidak_berserakan === "1"
                : null,
            "sampah_tempat_tertutup" => $request->has("sampah_tempat_tertutup")
                ? $request->sampah_tempat_tertutup === "1"
                : null,
            "sampah_dipilah" => $request->has("sampah_dipilah")
                ? $request->sampah_dipilah === "1"
                : null,
            "sampah_tidak_dibakar" => $request->has("sampah_tidak_dibakar")
                ? $request->sampah_tidak_dibakar === "1"
                : null,
            "sampah_tidak_dibuang_sembarangan" => $request->has(
                "sampah_tidak_dibuang_sembarangan",
            )
                ? $request->sampah_tidak_dibuang_sembarangan === "1"
                : null,
            "limbah_tidak_menggenang" => $request->has(
                "limbah_tidak_menggenang",
            )
                ? $request->limbah_tidak_menggenang === "1"
                : null,
            "limbah_saluran_tertutup" => $request->has(
                "limbah_saluran_tertutup",
            )
                ? $request->limbah_saluran_tertutup === "1"
                : null,
            "limbah_terhubung_resapan" => $request->has(
                "limbah_terhubung_resapan",
            )
                ? $request->limbah_terhubung_resapan === "1"
                : null,
        ];
        ...
        return back()->with(
            "success",
            "Checklist Kesehatan Lingkungan berhasil disimpan.",
        );
    }
}
