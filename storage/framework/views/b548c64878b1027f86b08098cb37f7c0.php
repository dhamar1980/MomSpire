<?php $__env->startSection('title', 'Konsultasi Dokter - MomSpire'); ?>
...
<div class="dokter-konsultasi-shell">
                ...
                <?php $__empty_1 = true; $__currentLoopData = $conversations; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $conversation): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); $__empty_1 = false; ?>
                    <?php
                        ...
                        if ($pengguna && $pengguna->last_seen) {
                            ...
                            if ($diff < 1) $lastSeenText = 'Baru saja';
                            elseif ($diff < 60) $lastSeenText = round($diff) . ' menit lalu';
                            elseif ($diff < 1440) $lastSeenText = floor($diff / 60) . ' jam lalu';
                            else $lastSeenText = \Carbon\Carbon::parse($pengguna->last_seen)->diffForHumans();
                        }
                    ?>
                    <div class="wa-conversation-item <?php echo e($selectedConversation && $selectedConversation->id === $conversation->id ? 'active' : ''); ?>"
                         data-conversation-id="<?php echo e($conversation->id); ?>"
                         data-conversation-name="<?php echo e(strtolower($conversation->pengguna->name ?? '')); ?>"
                         data-pengguna-online="<?php echo e($isOnline ? 'true' : 'false'); ?>"
                         data-pengguna-lastseen="<?php echo e($lastSeenText); ?>"
                         onclick="selectConversation(<?php echo e($conversation->id); ?>, <?php echo json_encode($conversation->pengguna->name ?? 'Pengguna', 15, 512) ?>, <?php echo e($isOnline ? 'true' : 'false'); ?>, <?php echo json_encode($lastSeenText, 15, 512) ?>)">
                        <div class="wa-conversation-avatar">
                            <?php echo e(strtoupper(substr($conversation->pengguna->name ?? 'P', 0, 1))); ?>

                        </div>
                        <div class="wa-conversation-info">
                            <div class="wa-conversation-name"><?php echo e($conversation->pengguna->name ?? 'Pengguna'); ?></div>
                            <div class="wa-conversation-preview"><?php echo e($lastSeenText ? 'Aktif ' . $lastSeenText : 'Offline'); ?></div>
                        </div>
                        <div class="wa-conversation-meta">
                            <span class="wa-conversation-time"><?php echo e($conversation->last_message_at?->diffForHumans() ?? 'Baru'); ?></span>
                            <?php if(($conversation->unread_count ?? 0) > 0): ?>
                                <span class="wa-unread-badge"><?php echo e($conversation->unread_count); ?></span>
                            <?php endif; ?>
                        </div>
                    </div>
                ...
                <div class="wa-chat-header">
                    ...
                    <div class="wa-chat-header-info">
                        ...
                        <div class="wa-chat-header-status" id="chatStatus">
                            <span id="statusDot" style="width:8px; height:8px; background:#bbb; border-radius:50%; display:inline-block; margin-right:4px;"></span>
...
<script>
    let currentConversationId = <?php echo e($selectedConversation?->id ?? 'null'); ?>;
    let currentConversationName = <?php echo json_encode($selectedConversation?->pengguna?->name ?? 'Pengguna', 15, 512) ?>;
    let firebaseUnsubscribe = null;

    const dokterId = <?php echo e(auth()->id()); ?>;
    const dokterName = <?php echo json_encode(auth()->user()->name ?? 'Dokter', 15, 512) ?>;

    <?php if($selectedConversation): ?>
        window.addEventListener('load', function() {
            selectConversation(<?php echo e($selectedConversation->id); ?>, <?php echo json_encode($selectedConversation->pengguna->name ?? 'Pengguna', 15, 512) ?>);
        });
    <?php endif; ?>
    ...
    async function selectConversation(conversationId, name, isOnline = false, lastSeen = '') {
        ...
        document.getElementById('chatName').textContent = name || 'Pengguna';

        // Update header status based on online status
        ...
        if (isOnline) {
            ...
            statusText.textContent = lastSeen ? 'Aktif ' + lastSeen : 'Offline';
        }
    ...
    async function loadChat(conversationId, name) {
        try {
            const response = await fetch(`<?php echo e(url('/dokter/konsultasi')); ?>/${conversationId}/messages`);
    ...
    async function sendMessage() {
        ...
        try {
            const response = await fetch('<?php echo e(route("dokter.konsultasi.send_message")); ?>', {
                method: 'POST',
                headers: {
            ...
            if (data.success) {
                ...
                if (typeof window.MomSpireChat !== 'undefined' && window.MomSpireChat.isReady()) {
                    window.MomSpireChat.sendMessage(`pengguna_dokter_${dokterId}`, 'dokter', dokterId, message, dokterName);
                }
            ...
            optMsg.querySelector('.wa-read-check').style.color = '#e63980';
        }
    }
    ...
    function setupFirebaseListener() {
        ...
        if (typeof window.MomSpireChat !== 'undefined' && window.MomSpireChat.isReady()) {
            window.MomSpireChat.setCurrentUser(dokterId, 'dokter');
            firebaseUnsubscribe = window.MomSpireChat.listenToMessages(`pengguna_dokter_${dokterId}`, function(messages) {
    ...
    document.addEventListener('DOMContentLoaded', function() {
        console.log('[Dokter Chat] Ready!');
    });
</script>
<?php $__env->stopPush(); ?>
<?php echo $__env->make('dokter.master', array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?><?php /**PATH C:\xampp\htdocs\MomSpire\resources\views\dokter\konsultasi.blade.php ENDPATH**/ ?>