...
@section('title', 'Jadwal Dokter - MomSpire')
@section('header_title', 'Jadwal')
@section('header_subtitle', 'Atur jadwal kontrol dan imunisasi yang akan dilihat pengguna.')
    ...
    <div class="section-card p-4 p-lg-5 mb-4">
        <div class="row align-items-center g-3">
            ...
            <div class="col-lg-4 text-lg-end">
                <form method="GET" action="{{ route('dokter.jadwal') }}" class="d-flex gap-2">
                    <input type="search" name="q" value="{{ $search }}" class="form-control" placeholder="Cari nama pengguna">
    ...
    <div class="row g-4">
        <div class="col-lg-5">
            <div class="section-card p-4 h-100">
                ...
                <form method="POST" action="{{ route('dokter.jadwal.store') }}" id="jadwalForm">
                    @csrf
                    ...
                    <div class="mb-3">
                        ...
                        <select name="pengguna_id" id="penggunaId" class="form-select" required>
                            ...
                            @foreach($penggunaList as $pengguna)
                                <option value="{{ $pengguna->id }}">{{ $pengguna->name }} {{ $pengguna->is_hamil ? '(Hamil)' : '' }}</option>
                            @endforeach
                        </select>
                    </div>
                    <div class="row g-3">
        ...
        <div class="col-lg-7">
            <div class="section-card p-4 h-100">
                <div class="d-flex justify-content-between align-items-center flex-wrap gap-2 mb-3">
                    ...
                    <span class="text-muted small">{{ $jadwalList->count() }} jadwal</span>
                </div>
                ...
                @forelse($jadwalList as $jadwal)
                    <div class="schedule-row mb-3">
                        <div class="d-flex justify-content-between align-items-start gap-2 flex-wrap">
                            <div>
                                <div class="fw-bold">{{ $jadwal->pengguna->name ?? 'Pengguna' }} - {{ $jadwal->judul }}</div>
                                <div class="text-muted small">{{ ucfirst($jadwal->jenis) }} • {{ $jadwal->tanggal->format('d M Y') }}{{ $jadwal->waktu ? ' • ' . $jadwal->waktu : '' }}</div>
                                <div class="text-muted small">{{ $jadwal->catatan ?: 'Tanpa catatan' }}</div>
                            </div>
                            <div class="text-end">
                                <span class="badge text-bg-{{ $jadwal->status === 'terjadwal' ? 'primary' : ($jadwal->status === 'selesai' ? 'success' : 'secondary') }} mb-2">{{ ucfirst($jadwal->status) }}</span>
                                <div>
                                    <button
                                        type="button"
                                        class="btn btn-sm btn-outline-primary edit-jadwal-btn"
                                        data-jadwal-id="{{ $jadwal->id }}"
                                        data-pengguna-id="{{ $jadwal->pengguna_id }}"
                                        data-jenis="{{ $jadwal->jenis }}"
                                        data-status="{{ $jadwal->status }}"
                                        data-judul="{{ e($jadwal->judul) }}"
                                        data-tanggal="{{ $jadwal->tanggal->format('Y-m-d') }}"
                                        data-waktu="{{ $jadwal->waktu ? substr((string) $jadwal->waktu, 0, 5) : '' }}"
