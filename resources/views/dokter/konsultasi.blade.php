...
@section('title', 'Konsultasi Dokter - MomSpire')
...
<script>
    ...
    const dokterId = {{ auth()->id() }};
    const dokterName = @json(auth()->user()->name ?? 'Dokter');

    @if($selectedConversation)
        window.addEventListener('load', function() {
            selectConversation({{ $selectedConversation->id }}, @json($selectedConversation->pengguna->name ?? 'Pengguna'));
        });
    @endif
    ...
    async function loadChat(conversationId, name) {
        try {
            const response = await fetch(`{{ url('/dokter/konsultasi') }}/${conversationId}/messages`);
        ...
        if (messages.length === 0) {
            ...
            messages.forEach((msg) => {
                ...
                if (msgDate && msgDate !== lastDate) {
                    html += `<div class="wa-date-separator"><span>${formatDateSeparator(messageDate)}</span></div>`;
                    lastDate = msgDate;
                }
    ...
    async function sendMessage() {
        ...
        try {
            const response = await fetch('{{ route("dokter.konsultasi.send_message") }}', {
                method: 'POST',
                headers: {
