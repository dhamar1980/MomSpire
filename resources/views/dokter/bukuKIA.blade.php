        ...
        <div class="col-12">
            <div style="border: 1px solid rgba(15, 23, 42, 0.08); border-radius: 18px; background: #fff; box-shadow: 0 10px 30px rgba(15, 23, 42, 0.04); padding: 2rem;">
                ...
                @if($templates->count() > 0)
                    ...
                    <form id="tambahBukuForm" method="POST" action="{{ route('dokter.bukuKIA.store') }}" class="d-none">
                        @csrf
                            ...
                            <div class="buku-list-item p-3 border-bottom">
                                <div class="d-flex justify-content-between align-items-start flex-wrap gap-2">
                                    ...
                                    <div class="d-flex gap-2">
                                        ...
                                        <form action="{{ route('dokter.bukuKIA.delete', $buku->id) }}" method="POST" style="display: inline;" onsubmit="return confirm('Yakin hapus?')">
                                            @csrf
                                            @method('DELETE')
...
<script>
    ...
    function editBuku(id, judul, catatan) {
        document.getElementById('editBukuForm').action = '{{ route("dokter.bukuKIA.update", ":id") }}'.replace(':id', id);
