...
@section('title', 'Kelola Buku KIA - ' . ($pengguna->name ?? 'Pengguna') . ' - MomSpire')
@section('header_title', 'Kelola Buku KIA')
@section('header_subtitle', 'Input dan kelola data Buku KIA untuk pengguna.')
@section('header_action')
<a href="{{ route($role . '.pengguna') }}" class="btn btn-back-page">
    ...
    <div class="pengguna-info-bar">
        <div class="d-flex flex-wrap justify-content-between align-items-center gap-3">
            <div class="d-flex align-items-center gap-3">
                <div>
                    <div class="pengguna-detail-name">{{ $pengguna->name }}</div>
                    <span class="badge {{ $pengguna->is_hamil ? 'bg-danger-subtle text-danger' : 'bg-secondary-subtle text-secondary' }} mt-2">
                        {{ $pengguna->is_hamil ? 'Hamil' : 'Tidak Hamil' }}
                    </span>
                </div>
            </div>
            <div class="d-flex gap-4">
                <div><small class="text-muted text-uppercase fw-bold" style="font-size:0.72rem;">Email</small><div>{{ $pengguna->email ?? '-' }}</div></div>
                <div><small class="text-muted text-uppercase fw-bold" style="font-size:0.72rem;">Total Anak</small><div>{{ $pengguna->anak_count ?? 0 }} anak</div></div>
            </div>
        </div>
    </div>
        ...
        <div class="kia-selector-body">
            ...
            <div class="kia-card-list">
                ...
                <div class="kia-card-item {{ $selectedKiaId == $kia->id ? 'active' : '' }}" data-kia-id="{{ $kia->id }}">
                    ...
                    <div class="kia-card-title">{{ $kia->ibu->nama ?? 'Buku KIA #' . $kia->id }}</div>
                    @if($kia->ibu && $kia->ibu->tanggal_lahir)
                    <div class="kia-card-subtitle">TTL: {{ $kia->ibu->tempat_lahir ?? '-' }}, {{ \Carbon\Carbon::parse($kia->ibu->tanggal_lahir)->format('d/m/Y') }}</div>
                    @endif
                    <span class="kia-card-badge {{ $kia->ibu && $kia->ibu->tanggal_lahir ? 'aktif' : 'draft' }}">
    ...
    <div class="kia-form-section" id="form-section">
        <div class="kia-form-header">
            ...
            <span class="badge" style="background: rgba(230, 57, 128, 0.08); color: #e63980;">
                <i class="bi bi-book me-1"></i> {{ $selectedKia->ibu->nama ?? 'Buku KIA #' . $selectedKia->id }}
            </span>
        </div>
...
<script>
    ...
    function pilihBukuKIA(id) {
        window.location.href = '{{ route($role . ".pengguna.bukuKIA.input", $pengguna->id) }}?buku_id=' + id;
    }

    function tambahBukuKIA() {
        if (confirm('Apakah Anda yakin ingin membuat buku KIA baru untuk pengguna ini?')) {
            fetch('{{ route($role . ".pengguna.bukuKIA.tambah", $pengguna->id) }}', {
                method: 'POST',
                headers: {
            ...
            .then(function(data) {
                if (data.success) {
                    window.location.href = '{{ route($role . ".pengguna.bukuKIA.input", $pengguna->id) }}?buku_id=' + data.id;
                }
            })
